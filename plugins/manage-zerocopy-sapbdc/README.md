...
This skill is **bundled with Cortex Code** — it ships from `snowflake-eng/cortex-code-skills` via `prod_manifest.yaml`, so most users do not need to install it manually. To iterate on it locally, clone this repo and use the skill folder directly:

```
git clone git@github.com:snowflake-eng/cortex-code-skills.git
cd cortex-code-skills/manage-zerocopy-sapbdc
```
