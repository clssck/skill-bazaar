...
Check the cache for the database and schema used by your deployment:

```bash
cat ~/.snowflake/cortex/memory/openflow_infrastructure_*.json | jq '.deployments[].runtimes[] | {name: .runtime_name, database: .runtime_db, schema: .runtime_schema}'
```
...
Replace `<execute_as_role>` with the runtime's execute_as_role from the cache.
