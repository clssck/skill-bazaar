...
def generate_stock_data_query(tickers: list[str], days: int) -> tuple[str, list]:
    """SQL text and positional parameters for synthetic stock price rows.

    All dynamic values use ``?`` binds: per-ticker fields, then ``days`` for the
    generator row count and the lookback used in ``DATEADD`` (same value twice).
    """
    row_placeholders = ", ".join(["(?, ?, ?, ?)" for _ in tickers])
    params: list = []
    for ticker in tickers:
        ...
        params.extend([ticker, base_price, growth_rate, volatility])
    params.append(days)
    params.append(days)
    ...
    raw_prices AS (
        SELECT 
            d.trade_date,
            t.ticker,
            t.base_price * POWER(1 + t.growth_rate, DATEDIFF(day, DATEADD(day, -?, CURRENT_DATE()), d.trade_date)) 
                * (1 + (RANDOM() / 10000000000000000000.0 - 0.5) * t.volatility * 2) AS close_price
        FROM date_series d
        CROSS JOIN tickers t
        WHERE DAYOFWEEK(d.trade_date) NOT IN (0, 6)
    )
    SELECT 
        trade_date AS date,
        ticker,
...
def load_stock_data(tickers: list[str], days: int) -> pd.DataFrame:
    ...
    query, params = generate_stock_data_query(tickers, days)
