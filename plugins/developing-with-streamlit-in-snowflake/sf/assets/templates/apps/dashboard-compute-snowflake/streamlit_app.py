...
# =============================================================================
# SQL safety: bind parameters (?) for values; identifiers are allowlisted only.
# =============================================================================
...
def build_synthetic_query(
    ...
    """Build SQL and positional bind parameters for synthetic category data.

    Category labels and numeric bases are passed as ``?`` binds (values only).
    ``category_col`` is interpolated only after identifier validation.
    """
    _validate_sql_identifier(category_col)

    value_placeholders = ", ".join(["(?, ?)" for _ in categories])
    params: list = []
    for cat in categories:
        params.append(cat)
        params.append(base_values.get(cat, 1000))
    params.append(DATE_SERIES_ROWCOUNT)
...
def load_account_type_data() -> pd.DataFrame:
    ...
    query, params = build_synthetic_query(
...
def load_region_data() -> pd.DataFrame:
    ...
    conn = get_snowflake_connection()
    query, params = build_synthetic_query("region", REGIONS, BASE_VALUES["region"])
    df = conn.query(query, params=params)
    df.columns = df.columns.str.lower()
    return df
