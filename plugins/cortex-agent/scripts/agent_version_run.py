#!/usr/bin/env python3
"""
Run a specific version of a Cortex Agent and save the response.

This script sends a request to the versioned run endpoint of a Snowflake
Cortex Agent and saves the response to a file.
"""
...
def encode_version(version: str) -> str:
    """URL-encode the version parameter.

    The '$' in VERSION$N must be encoded as '%24' in URL paths.

    Args:
        version: Version identifier (e.g., VERSION$2, LIVE, DEFAULT, production)

    Returns:
        URL-safe version string
    """
    return version.replace("$", "%24")


def run_agent_version(agent_name, version, question, output_file,
    ...
    """
    Send a request to a specific version of an agent and save the response.

    Args:
        agent_name: Name of the agent
        version: Version to run (VERSION$N, alias, LIVE, DEFAULT, FIRST, LAST)
    ...
    try:
        ...
        url = f"https://{host}/api/v2/databases/{database}/schemas/{schema}/agents/{agent_name}/versions/{encoded_version}:run"
        ...
        print(f"Sending request to agent {database}.{schema}.{agent_name} version={version}")
        print(f"Question: '{question}'")
        print(f"Streaming response...\n")

        response = requests.post(url, headers=headers, json=payload, stream=True, verify=False, timeout=(30, 900))
        ...
        if response.status_code != 200:
            ...
            if final_response:
                ...
                print("Error: No final response received", file=sys.stderr)
                return None
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run a specific version of a Cortex Agent and save the response",
        epilog="""
Examples:
  # Run the LIVE version
  %(prog)s --agent-name MY_AGENT --version LIVE --question "What can you do?" --output-file ./response.json

  # Run a committed version (VERSION$2)
  %(prog)s --agent-name MY_AGENT --version "VERSION\$2" --question "What can you do?" --output-file ./response.json

  # Run the DEFAULT version
  %(prog)s --agent-name MY_AGENT --version DEFAULT --question "What can you do?" --output-file ./response.json

  # Run a user-defined alias
  %(prog)s --agent-name MY_AGENT --version production --question "What can you do?" --output-file ./response.json

  # Run with workspace auto-resolution
  %(prog)s --agent-name MY_AGENT --version LIVE --question "What can you do?" \\
    ...
    parser.add_argument("--version", required=True,
                        help="Version to run: VERSION$N, alias, LIVE, DEFAULT, FIRST, or LAST")
    ...
    parser.add_argument("--enable-research-mode", action="store_true", help="Enable research mode (default: False)")
    ...
    try:
        result = run_agent_version(args.agent_name, args.version, args.question, output_path, args.database, args.schema, args.connection, current_date_override=args.current_date_override, enable_research_mode=args.enable_research_mode)
        if result is None:
            sys.exit(1)
