...
| 13 | PSSv2 silently breaks Glue Iceberg CLDs | Snowpipe Streaming v2 incompatibility |
| 14 | Crawler creates spurious entries for Iceberg data | Glue S3 crawler + Iceberg format |
| 15 | ALLOWED_NAMESPACES causes error 300010 | CLD creation with Glue IRC + external volume |
...
## 13. Snowpipe Streaming v2 silently breaks catalog-linked Glue Iceberg tables

**Symptom**: After enabling Snowpipe Streaming v2 (PSSv2), catalog-linked tables backed by Glue Iceberg stop updating or return stale/incorrect data — with no obvious error.

**Root cause**: PSSv2 only supports `CATALOG='SNOWFLAKE'`. It is incompatible with external catalog types (including `CATALOG_API_TYPE = AWS_GLUE`). Enabling PSSv2 on a pipe that feeds catalog-linked tables silently corrupts the write path.

**Snowflake docs**: [Snowpipe Streaming — limitations](https://docs.snowflake.com/en/user-guide/data-load-snowpipe-streaming-overview#limitations): "PSSv2 is supported only for Iceberg tables with `CATALOG = 'SNOWFLAKE'`."

**Mitigation**:
- Do NOT use PSSv2 for pipelines that write to Glue-backed Iceberg tables
- If you need streaming ingest into Glue Iceberg, use standard Snowpipe (v1) or write directly via Spark/Flink on the AWS side and let the CLD auto-refresh pick it up
- If PSSv2 was already enabled, disable it and verify table consistency before resuming
...
## 14. Glue S3 crawler creates spurious entries for Iceberg data

**Symptom**: After crawling an Iceberg S3 path, Glue creates 3–5 unexpected tables with names like `metadata`, `data`, `snap_*`, or `manifest_list_*`. The real Iceberg table is either missing or misnamed.

**Root cause**: The standard Glue S3 crawler doesn't understand Iceberg format. It treats `.metadata.json` files, Avro manifests, and data directories as separate tables. It has no concept of the Iceberg table hierarchy.

**Fix**:
1. Delete the spurious tables:
   ```bash
   aws glue delete-table --database-name <DB_NAME> --name <SPURIOUS_TABLE> \
     --profile <PROFILE> --region <REGION>
   ```
2. Register the correct Iceberg table manually using `aws glue create-table` with `Parameters.table_type = ICEBERG` and a full `StorageDescriptor`. See Phase 3.4 in `aws-setup/SKILL.md` for the canonical registration template.

**Prevention**: For Iceberg-format source data (option A in workflow routing), skip the crawler entirely and go straight to manual registration.

---

## 15. `ALLOWED_NAMESPACES` causes internal error 300010 with Glue IRC + external volume

**Symptom**: `CREATE DATABASE ... LINKED_CATALOG` fails with Snowflake internal error `300010` when `ALLOWED_NAMESPACES` is specified.

**Root cause**: This appears to be a Snowflake bug. When using `CATALOG_API_TYPE = AWS_GLUE` with `ACCESS_DELEGATION_MODE = EXTERNAL_VOLUME_CREDENTIALS`, the `ALLOWED_NAMESPACES` filter triggers an internal error. The CLD creation fails entirely.

**Workaround**: Remove `ALLOWED_NAMESPACES` from the `CREATE DATABASE ... LINKED_CATALOG` statement. The CLD will sync all Glue databases visible to the catalog integration — use `BLOCKED_NAMESPACES` instead to exclude specific databases you don't want:

```sql
-- Instead of ALLOWED_NAMESPACES = ('target_db'), use:
CREATE DATABASE <cld_name>
  FROM CATALOG INTEGRATION = <integration_name>
  EXTERNAL VOLUME = <external_volume>
  BLOCKED_NAMESPACES = ('unwanted_db1', 'unwanted_db2');
-- Or omit namespace filtering entirely — see KNOWN_GOTCHAS #12 for impact
```

**Note**: This issue does not affect `VENDED_CREDENTIALS` mode. If you need strict namespace filtering with external volume mode, consider using IAM-level scoping (scope the IAM role's Glue permissions to specific databases/tables).
