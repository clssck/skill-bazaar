...
Drops (deletes) a DCM project object **metadata only**.
...
**`snow dcm drop` removes only the DCM project registration — it does NOT drop the Snowflake objects managed by the project** (tables, views, dynamic tables, tasks, etc.). Those objects remain in Snowflake as orphans after the project is dropped.

To drop the project AND all managed objects, use `snow dcm purge` instead (see below).

---

### `snow dcm purge`

**🚨 EXTREMELY DANGEROUS.** Permanently destroys every Snowflake object managed by the project, including all table data. This cannot be automatically undone.

> **Note:** `snow dcm purge` drops the managed objects but **does NOT drop the DCM project registration itself**. If you also want to remove the project metadata, run `snow dcm drop` after purge.

#### Syntax

```bash
snow dcm purge <identifier> -c <connection> [options]
```
...
| Argument     | Required | Description                               |
| ------------ | -------- | ----------------------------------------- |
| `identifier` | Yes      | The identifier of the DCM project to purge |
...
| ----------- | ---------------------------------------------------------------------------------- |
| `--target`  | Target from `manifest.yml` to use (bundles project identifier + templating config) |
| `--force`   | Skip the CLI's own interactive prompt (use after obtaining user confirmation)       |
| `--alias`   | Alias for the purge operation (for tracking/management)                            |

#### Examples

```bash
snow dcm purge MY_DB.MY_SCHEMA.MY_PROJECT -c myconnection --force
```

#### Agent Guidance

**NEVER run this command without going through the full purge workflow in [purge-project/SKILL.md](../purge-project/SKILL.md).** The workflow requires:

1. Enumerating managed objects (via `raw-analyze` if local files exist, or `SHOW ENTITIES IN DCM PROJECT` as fallback)
2. Presenting the full object list with an explicit danger warning
3. Receiving explicit user confirmation ("yes") before executing
4. Always passing `--force` so the CLI does not re-prompt after you've confirmed with the user
