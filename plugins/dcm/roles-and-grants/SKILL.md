...
## DRY Patterns: Using Macros for Repeated Roles/Grants

When the **same role/grant pattern** repeats across multiple teams, schemas, or entities, define it once as a Jinja macro and call it in a `{% for %}` loop. Load [../reference/primitives/jinja_templating.md](../reference/primitives/jinja_templating.md) for full syntax before writing macros.
...
8. **Use macros for repeated patterns** - When the same role/grant structure applies to multiple teams or schemas, define it once as a Jinja macro and call it in a loop
