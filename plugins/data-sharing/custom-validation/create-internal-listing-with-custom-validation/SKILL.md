---
name: create-internal-listing-with-custom-validation
description: >
...
6. **Run validation gate** — read and follow [`run-listing-custom-validation/SKILL.md`](run-listing-custom-validation/SKILL.md) for all listings; pass whether this is a draft or publish as context. Do NOT skip or substitute with another query.
7. **Create and publish** immediately after confirmation
...
```
Start → Step 1: Gather → Step 2: Create Share → Step 3: Build Manifest → 🔒 Validation Gate → Step 4: Execute & Verify → Done
            ↑                                         ↑                          ↑                       ↑
      ⚠️ STOP                                   ⚠️ STOP                   ⚠️ STOP             ⚠️ STOP (publish only)
```
...
1. **Check if share already exists, then create**:

   First, check for an existing share with the same name:
   ```sql
   SHOW SHARES LIKE '<share_name>';
   ```

   **If the share already exists** (result is non-empty), stop and inform the user:
   ```
   A share named "<share_name>" already exists. How would you like to proceed?
   1. Choose a new share name — I'll use that instead
   2. Reuse the existing share — I'll skip creation and proceed with granting objects to it
   ```
   **⚠️ STOP**: Wait for the user's choice before continuing.
   - **Option 1**: Ask the user for the new name, then re-run `SHOW SHARES LIKE '<new_share_name>'` to confirm it doesn't already exist. Replace `<share_name>` with the new name for every remaining step in this workflow — including all GRANT statements, DESCRIBE SHARE, and the CREATE ORGANIZATION LISTING manifest.
   - **Option 2**: Skip the `CREATE SHARE` command and proceed directly to granting privileges.
...
Does this look correct? If so, I'll run a validation check before creating the listing. (Yes/No)
```
...
**Goal:** Validate the listing manifest before executing the `CREATE ORGANIZATION LISTING` SQL. No listing is created yet at this point.

**⚠️ MANDATORY**: Read [`run-listing-custom-validation/SKILL.md`](run-listing-custom-validation/SKILL.md) and follow its complete workflow now. Pass the listing manifest from Step 3 as context. Run this validation check before executing the `CREATE ORGANIZATION LISTING` SQL. Do NOT substitute this with any other SQL query or skip to Step 4.

- **If `PUBLISH = TRUE`**: failures block execution — do not proceed to Step 4 until resolved.
- **If `PUBLISH = FALSE`** (draft): show validation results as a preview but proceed regardless of outcome.

This gate runs automatically for all listings — do not skip it even if the user doesn't mention validation. If there are no active rules, it will pass through immediately.

**Only proceed to Step 4 after the child skill's workflow completes.**
