...
**⚠️ Requires `MANAGE LISTING AUTO FULFILLMENT` on ACCOUNT.** Run the preflight first — see `../workflows/org-listing.md` Step 0.
...
## Template: Cron-Based Refresh Schedule

**⚠️ Requires `MANAGE LISTING AUTO FULFILLMENT` on ACCOUNT.** Run the preflight first — see `../workflows/org-listing.md` Step 0a.

Use when the user asks for a specific time/day pattern rather than a fixed interval. Format: `USING CRON <minute> <hour> <day_of_month> <month> <day_of_week> <IANA_timezone>`.

```sql
-- Example: refresh weekdays at 5pm London time
CREATE ORGANIZATION LISTING weekday_listing
  SHARE my_share AS
$$
title: "Weekday-Refreshed Data Product"
description: "Refreshed Monday-Friday at 5pm London"
organization_targets:
  discovery:
    - all_internal_accounts: true
  access:
...
### Updating refresh schedule on an existing listing

`ALTER LISTING` replaces the entire manifest, so read the existing manifest first with `DESCRIBE LISTING`, change only `auto_fulfillment.refresh_schedule`, and submit. Do not hand-write placeholder values for unrelated fields.

```sql
-- 1. Read the existing manifest. The YAML is in the "manifest_yaml" column.
--    Use REVISION = DRAFT if you want to preserve unpublished draft edits.
DESCRIBE LISTING weekday_listing;

-- 2. Take the "manifest_yaml" value as-is, change ONLY auto_fulfillment.refresh_schedule
--    to the new value (e.g. "USING CRON 0 6 * * * America/New_York" or "60 MINUTE").

-- 3. Submit the updated manifest:
ALTER LISTING weekday_listing AS
$$
...
**⚠️ Do NOT use these invalid forms** (they will fail):
- `ALTER LISTING my_listing SET refresh_schedule = ...`
- `ALTER DATABASE my_db SET refresh_schedule = ...`
...
`ALTER LISTING` uses `AS` without `SET` — and it replaces the ENTIRE manifest, so pass the full YAML:
...
```sql
-- Temporarily hide (use UNPUBLISH, not SET STATE = UNPUBLISHED)
...
-- The YAML manifest is in the "manifest_yaml" column of DESCRIBE LISTING.
-- Use REVISION = DRAFT to see unpublished draft edits:
DESCRIBE LISTING my_listing REVISION = DRAFT;
```
